...
def screenplay_required_duration_s(screenplay, *, minimum_s: int = 0) -> int:
    ...
    if getattr(screenplay, "narrative_plan", None) is not None:
        ...
        # #region debug-point A:authoritative-duration
        try:
            ...
            _debug_durations: dict[str, int] = {}
            for _debug_shot in outline.shots:
                _debug_key = str(int(_debug_shot.duration_s or 0))
                _debug_durations[_debug_key] = (
                    _debug_durations.get(_debug_key, 0) + 1
                )
            _debug_spine = getattr(screenplay, "plot_spine", None)
            _debug_beats = list(
                getattr(_debug_spine, "spine_beats", None) or []
            )
            _debug_request.urlopen(
                _debug_request.Request(
                    ...
                    data=_debug_json.dumps({
                        ...
                        "hypothesisId": "C42-A",
                        "location": (
                            "app/renderability.py:"
                            "screenplay_required_duration_s"
                        ),
                        "msg": (
                            "[DEBUG] authoritative screenplay duration inputs"
                        ),
                        "data": {
                            "minimum_s": int(minimum_s or 0),
                            "spine_beat_count": len(_debug_beats),
                            "must_keep_spine_beat_count": sum(
                                1 for _debug_beat in _debug_beats
                                if _debug_beat.must_keep
                            ),
                            "screenplay_event_count": len(
                                getattr(screenplay, "events", None) or []
                            ),
                            ...
                            "outline_shot_count": len(outline.shots),
                            "shot_duration_histogram": _debug_durations,
                            "authoritative_duration_s": (
                                authoritative_outline_duration_s(outline)
                            ),
                            "compile_audit": _audit,
                        },
