...
以下三处「运维时必须绕开产品」是**已知且有意**的，不是遗漏。判据来自一条经验：
验收/运维脚本里每出现一次裸 SQL，就标记一处「用户做不到的事」——所以这类绕过
必须逐条给出裁决，而不是默认它合理。同一次扫描查出的第四处（停用团队只能改库）
确实是缺口，已补 ``PUT /workspaces/{workspace_id}``。

1. **首个系统管理员靠 scripts/create_admin.py 直接写库**。先有鸡后有蛋：没有管理员
   就没人能调用本模块的开户接口。这条无法也不该在产品内解决。
2. **没有「删除用户/删除团队」，只有停用**。刻意如此：审计表里的 decided_by /
   archived_by 指向 users.id，删账号会让历史操作失去归属；停用已经达到「此人不能
   再登录、其团队成员立即失权」的效果，且可逆。代价是用户名被永久占用（UNIQUE），
   打错字建的账号只能停用不能回收——接受这个代价，换审计完整性。
3. **签发服务账号长效凭证（如回归脚本用的 data/regression_session_token.txt）没有
   产品入口**，只能由管理员登录后调 create_session 写文件。这是真实的运维缺口，
   但本期不补：MCP token（``/api/system/mcp-tokens``）已经是"给非人类调用方发凭证"
   的产品化路径，正确解法是让服务账号改用 Bearer 而不是再造一套长效会话。
   记在案，避免日后有人以为这里"忘了做"。
"""
