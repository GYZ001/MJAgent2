...
async def run_storyboard_supervisor(
    ...
    *,
    ...
    if narrative_authority:
        try:
            import json as _dbg_json, urllib.request as _dbg_request; from app.narrative import validate_screenplay_narrative as _dbg_validate, validate_storyboard_screenplay_authority as _dbg_runtime_validate; _dbg_p=".dbg/storyboard-semantic-outline-failure.env"; _dbg_u,_dbg_s="http://127.0.0.1:7777/event","storyboard-semantic-outline-failure"; _dbg_c=open(_dbg_p).read(); _dbg_u=next((line.split("=",1)[1] for line in _dbg_c.splitlines() if line.startswith("DEBUG_SERVER_URL=")),_dbg_u); _dbg_s=next((line.split("=",1)[1] for line in _dbg_c.splitlines() if line.startswith("DEBUG_SESSION_ID=")),_dbg_s); _dbg_errors=_dbg_validate(screenplay,require=True,expected_scope_id=episode_id); _dbg_runtime_errors=_dbg_runtime_validate(screenplay,expected_scope_id=episode_id); _dbg_request.urlopen(_dbg_request.Request(_dbg_u,data=_dbg_json.dumps({"sessionId":_dbg_s,"runId":"post-fix","hypothesisId":"A,C,E","location":"app/storyboard_supervisor.py:run_storyboard_supervisor","msg":"[DEBUG] Published screenplay graph before storyboard","data":{"episodeId":episode_id,"scoreOnlyErrorCount":len(_dbg_errors),"runtimeErrorCount":len(_dbg_runtime_errors),"runtimeErrorCodes":[item.partition("]")[0].lstrip("[") if item.startswith("[") else "UNKNOWN" for item in _dbg_runtime_errors],"hasPublishedShots":bool(ep["published_storyboard_artifact_id"])},"ts":int(__import__("time").time()*1000)}).encode(),headers={"Content-Type":"application/json"}),timeout=0.5).read()
        except Exception:
            pass
    # #endregion
    if narrative_authority:
