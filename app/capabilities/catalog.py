...
def _register_commands(registry) -> None:
    commands = [
        ...
        _cmd(
            "screenplay.generate",
            title="生成可交付剧本",
            description="一次 Baseline 生成 + 局部自愈；已有 published 时拒绝 force 全量重生",
        ...
        _cmd(
            "screenplay.revise",
            title="复验已发布剧本",
            description="从已发布版本克隆工作副本，按当前规则复验并仅用局部 Patch 收敛",
            input_model=I.ScreenplayReviseInput,
            risk=RiskLevel.R2_MATERIAL,
            confirmation=ConfirmationPolicy.WHEN_IMPACT,
            idempotency=IdempotencyPolicy.REQUIRED,
            scopes={"manju:generation-text"},
            side_effect="creates_working_revision",
            handler=h_screenplay.revise,
            rest_routes=("POST /api/episodes/{episode_id}/screenplay/revise",),
            supports_cancel=True,
            tags=("screenplay", "production"),
        ),
        _cmd(
        ...
        _cmd(
            "screenplay.resume",
            title="继续剧本局部修复",
            description="从已有 working Artifact 和 checkpoint 继续局部 Patch，禁止再次整版生成",
            input_model=I.ScreenplayResumeInput,
            risk=RiskLevel.R2_MATERIAL,
            confirmation=ConfirmationPolicy.WHEN_IMPACT,
            idempotency=IdempotencyPolicy.RECOMMENDED,
            scopes={"manju:generation-text"},
            side_effect="resumes_working_revision_patch_loop",
        ...
        _cmd(
            "storyboard.clear",
            title="一键清空分镜",
            description="停止本集分镜任务并永久删除镜头、检查点、分镜缓存、媒体和运行记录；保留剧本",
            input_model=I.EpisodeScopedInput,
            risk=RiskLevel.R3_DESTRUCTIVE,
            confirmation=ConfirmationPolicy.ALWAYS,
            idempotency=IdempotencyPolicy.REQUIRED,
            scopes={"manju:project-write"},
            side_effect="permanently_deletes_storyboard_and_downstream_products",
            handler=h_storyboard.clear,
            rest_routes=("DELETE /api/episodes/{episode_id}/storyboard",),
            tags=("storyboard", "destructive"),
        ),
        _cmd(
            "storyboard.cancel",
            title="取消分镜",
        ...
        _cmd(
            "storyboard.set_shot_adoption",
            title="设置分镜采纳状态",
            description="恢复或取消单镜采纳；未采纳分镜不进入后续媒体生产",
            input_model=I.StoryboardShotAdoptionInput,
            risk=RiskLevel.R1_REVERSIBLE,
            confirmation=ConfirmationPolicy.NEVER,
            idempotency=IdempotencyPolicy.RECOMMENDED,
            scopes={"manju:project-write"},
            side_effect="toggles_storyboard_downstream_inclusion",
            handler=h_storyboard.set_shot_adoption,
            rest_routes=("POST /api/shots/{shot_id}/storyboard-adoption",),
            tags=("storyboard", "shot", "gate"),
        ),
        # —— 视频 / 参考图 ——
        _cmd(
