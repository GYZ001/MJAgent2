...
被 ``function_lines`` 闸门当成「函数变复杂」——与 ``app/capabilities/commands/
system.py`` 同一处理（2026-09-23 新增 ``project.update_settings`` 时顺手把这一条
也做成数据，之前逐条 ``_cmd`` 堆在 ``commands()`` 里已经把它的 function_lines
顶到基线 72，再加一条会撞线）。
"""
...
_COMMAND_SPECS: tuple[CommandSpec, ...] = (
    ...
    _cmd(
        # 项目级设置（2026-09-23 新增）：改编强度档位/画幅/AI 标识，见
        # app.project_settings。风险档比照 system.set_engine（同为「项目级
        # 开关，影响此后新生成内容」），不是删除资源，NEVER 确认。
        "project.update_settings",
        title="更新项目设置",
        description="更新改编强度档位/画幅/AI 标识——项目级设置，只影响此后新生成的分镜/成片，不改写已有产出",
        input_model=I.ProjectUpdateSettingsInput,
        risk=RiskLevel.R2_MATERIAL,
        confirmation=ConfirmationPolicy.NEVER,
        idempotency=IdempotencyPolicy.REQUIRED,
        scopes={"manju:project-write"},
        side_effect="updates_project_settings",
        handler=h_project.update_settings,
        rest_routes=("PUT /api/projects/{project_id}/settings",),
        tags=("project",),
    ),
)
