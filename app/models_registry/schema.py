...
def ensure_schema() -> None:
    ...
    连接、不抢锁（这次调用不触发下面的迁移，见后段说明）；否则保持原有独立
    连接行为（``db._run_write_transaction_once``），供 ``app.main`` 启动、
    ``app/models_registry/store.py`` 等各自的防御性调用使用。
    ...
    def _run_independent() -> None:
        ...
        # 最多触发一次」，不是迁移正确性的唯一保障。只在这条独立连接分支里触
