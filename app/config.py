...
# 都会把这种卡死空等拉满外层预算。
# 实测 run_59d372954c0e：成功点名首字最慢 19.4s，20s 上限把 12 次仍在排队的流误杀，
# 8/20 分块耗尽后整轮人物谱失败。60s 仍切断 300s 空等，并给并发点名留出余量。
# 只在调用方显式要求（disable_thinking / first_token_timeout_s）时生效。
TIMEOUT_CHAT_FIRST_TOKEN_S = float(os.environ.get("TIMEOUT_CHAT_FIRST_TOKEN_S", "60"))
