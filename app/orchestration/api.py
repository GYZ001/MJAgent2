...
def _current_delivery_download_manifest(conn, row) -> dict:
    ...
    from app.delivery import verify_delivery_package_artifact_binding

    try:
        verify_delivery_package_artifact_binding(
            conn,
            row,
            episode_id=str(row["episode_id"]),
            allowed_statuses={expected_artifact_status},
        )
    except ValueError as exc:
        raise HTTPException(409, f"交付包 Artifact 已失效：{exc}") from exc
    try:
