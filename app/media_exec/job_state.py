    ...
    if exc.failure.category is hiagent.ProviderFailureCategory.MODEL_REJECTION:
        ...
        if exc.failure.kind == PROVIDER_CONTENT_REJECTED_KIND:
            return (
                ...
                "系统已停止对本镜的自动重试，并把本镜按「跳过」处理：本集成片照常合成、不含本镜。需要补上本镜时，先看供应商原文指向的是画面描述还是台词：画面描述里有血腥/擦边字眼的，到分镜台选中本段，点「复核说话人和群演」，改「片段镜头稿」后校验预览并保存，再回生成台重新生成；画面描述没有这类字眼的，触发点就在台词原句上——台词逐字进提示词，改片段镜头稿救不了，只能改剧本这几句；换供应商没有只换一镜的做法——分镜台顶部的「视频模型（本集）」是整集切换，本集已生成的视频产物会被清空。直接点「重新生成」会原样重提同一段提示词，供应商仍会给出同样的拒绝。",
            )
        return (
            "VIDEO_PROVIDER_MODEL_REJECTED",
            f"当前视频模型明确拒绝了本次输入，系统已保持 {mode or '原计划模式'} 失败且没有改写内容或切换生成方式。{quote}"
            "系统已停止对本镜的自动付费重试，转人工处理。请到分镜台选中本段，点「复核说话人和群演」，把画面描述里的敏感字眼改掉，改「片段镜头稿」后校验预览并保存，再回生成台重新生成；若供应商拒绝的是台词原句，改片段镜头稿救不了，只能改剧本这几句；换供应商没有只换一镜的做法——分镜台顶部的「视频模型（本集）」是整集切换，本集已生成的视频产物会被清空。",
        )
    if (
        ...
        return (
            ...
            f"视频供应商执行失败，供应商原文：{provider_text}。系统已停止对本镜的自动付费重试，转人工处理。请到分镜台选中本段，点「复核说话人和群演」，改「片段镜头稿」后校验预览并保存，再回生成台重新生成；若怀疑是台词原句本身触发，改片段镜头稿救不了，只能改剧本这几句；换供应商没有只换一镜的做法——分镜台顶部的「视频模型（本集）」是整集切换，本集已生成的视频产物会被清空。",
        )
    return None
