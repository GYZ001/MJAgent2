...
问题。继续拆分需要显式依赖注入：2026-08-30 评估后结论不拆——``task_id``/``meta``/``prompt_text``
重新绑定，两层嵌套 try/except 编码原子性边界；循环后约 200 行尾段同样否掉，因技术重投在另一条 ``get_conn()``
连接自行提交。它仍是全部上述子模块的编排者，逐行搬移到任何一个子模块都会让该子模块
            ...
            if not task_id:
                ...
                if meta.get("mode") == video_modes.REFERENCE_IMAGE_MODE:
                    prompt_text = freeze_segment_reference_audios(
                        conn, job, version, shot, meta, prompt_text, operation_id=provider_operation_id)
