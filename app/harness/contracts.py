...
_CONTRACTS: dict[str, StageContract] = {
    ...
    "screenplay": StageContract(
        ...
        invariants=[
            ...
            "character decisions use only perceivable evidence",
            "every experience intent has an independent path for every audience prior",
            "failed narrative gates preserve a working artifact but never publish ready",
        ],
        max_iterations=4,
        requires_human_gate=True,
    ),
    "storyboard": StageContract(
        ...
        invariants=[
            ...
            "every shot owns a non-empty narrative contribution even when primary_action_id is null",
            "primary atomic actions and per-prior target deltas have one main delivery owner",
            "adjacent shots preserve per-prior audience-state and narrative-boundary handoffs",
            "readability windows meet scheduled processing demand after attention competition",
            "repair selection compares multiple semantic candidates and revalidates the whole episode",
            "multi-prior blind-audience review passes before storyboard publication",
        ],
        max_iterations=4,
    ),
    "video": StageContract(
