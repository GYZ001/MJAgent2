...
def normalize_scene_image_qa(qa: dict[str, Any] | None, *, environment_only: bool = True) -> dict[str, Any]:
    ...
    layout_detail_matches = _bool(result.get("layout_detail_matches"))
    material_contract_matches = _bool(result.get("material_contract_matches"))
    ...
    if material_contract_matches is False:
        hard.append("场景材质与本次场景合同不符")
    ...
    soft = [
        ...
        *(
            ["空间大类一致，但布局或陈设细节存在质检差异"]
            if layout_detail_matches is False else []
        ),
    ]
    ...
    result.update({
        ...
        "layout_detail_matches": layout_detail_matches,
        "material_contract_matches": material_contract_matches,
        "space_type_hard_failure": space_type_matches is False,
        "hard_failures": hard,
