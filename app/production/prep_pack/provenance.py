...
def _prep_pack_locate_stitched_quote(segments: list[SourceSegment], phrase: str) -> tuple[list[int], str]:
    ...
    parts = [x for x in (part.strip() for part in re.split(r"(?<=[。！？…．.!?])|[；;]", phrase)) if x and x.strip(_PREP_PACK_TERMINAL_MARKS + "…")]  # 「；」是多段引文的拼接符；只剩「…」的碎片不是句子（2026-09-15「周医生→周晚」两段引文以「；」相连、尾部「……」，整条被拒）
