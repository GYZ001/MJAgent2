"""Per-chunk extraction call (_extract_chunk) and its model-facing prompt
rules.

Split out of app/production/prep_pack.py. Step/telemetry bookkeeping and the
structured model-call wrapper (_begin_step/_finish_step/_run_sync_step/
_run_async_step/_call_structured) now live in ``.model_call``（架构复测
2026-09-23）：``scene_recheck.py`` 需要 ``_call_structured``，本模块又要在
``_extract_chunk`` 里调用 ``scene_recheck.attach_scene_recheck`` 补场景漏报，
留在这里会让两个兄弟模块互相 import 成环。下面用 `as` 自别名重新导出，全仓
既有的 ``from .chunk_extraction import _call_structured`` /
``app.production.prep_pack._call_structured`` 用法不变。
"""
...
from .model_call import (
    _begin_step as _begin_step,
    _call_structured as _call_structured,
    _finish_step as _finish_step,
    _run_async_step as _run_async_step,
    _run_sync_step as _run_sync_step,
)
from .schemas import _ChunkResponse


# 素材申报规则（纯静态文本，无插值）。抽成模块级常量而不是留在 _extract_chunk 的
# f-string 里：规则每增删一段就顶 function_lines 棘轮，而这块本来就不该算进函数复杂度。
#
# 2.0.5（真实案例，龙猫出爪 EP1 第 17 段：用户看到分镜没关联到本该匹配的「晚安宠物
# 医院门口」）新增末尾两段。全链路核查后每一环都没做错：剧本原文第 14 段的场次标题是
# 「【段 12｜人间·老街｜清晨】」，正文写「周晚站在门口，看向街对面」，那个「门口」是
# 她自家医院的门口，而完整写法只在第 3 段的场次标题「【段 01｜人间·晚安宠物医院·
# 门口｜傍晚】」里出现过。模型那次抽取报的 8 个场景 quote 全部是场次标题行，
# 「晚安宠物医院门口」只拿到 segment_indexes=[3]；分镜按段号交集拿到的候选集因此只有
# 「人间老街区」一个，分镜模型即使在散文里写出了「站在宠物医院门口」，resources.scenes
# 里也没有合法 scene_id 可填。
#
# 根因是本文件两条既有硬性规则的交互盲区：「场景的持续性」整条规则假设「一段原文里
# 人物所在的地点几乎总是连续的」，没有覆盖「一个编号的画面同时涉及两个地点」；
# 「命名纪律」要求 display_name 逐字取自本段原文，而省略说法（「门口」）指向的完整
# 写法在别的编号里。两条各自都对，合起来把这种情形挤成了没有合法答案的题。
#
# 两道既有闸门都放行新规则下的申报，动手前逐个验过：_prep_pack_gate_segment_indexes
# 只校验编号落在本 chunk 范围内，不要求 display_name 逐字出现在那些编号里；
...
那个完整写法申报，并把本编号计入它的 segment_indexes。这是下面 display_name 命名纪律的
唯一例外，要同时满足两件事才成立：完整写法确实在本次输入里逐字出现过（照抄那个写法，不要
自己改写或补全），并且本编号的省略说法确实指的就是它。把作为依据的那句省略说法原句填进
这条申报的 quote。两件事只要有一件不确定就不申报——漏掉一个地点，好过把人物放进一个他
没去过的地方。
...
- 场景地点的 display_name 一律使用原文自己的描述词，不得替换成你认为等价的其他
  ...
  真名假设同样走 suspected_true_name）；唯一的例外是上面"用别处的完整写法回指同一
  地点"那条，它同样只允许照抄本次输入里逐字出现过的写法，不允许自己造词或补全。"""
