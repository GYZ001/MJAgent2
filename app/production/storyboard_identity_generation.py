...
IDENTITY_GENERATION_RULES = [
    ...
    "subject_kind：known_character_identities 列出全书已确认角色正名及 identity_id；原文指向该角色本人时用 character，参考图为空也保持角色身份；独立无名人物用 extra；复数人群用 crowd。群演可以使用 relevant_assets.functional_extras[].visual_entity_id；未收录的无名人保留原文称谓并独立描述。身份由原文关系决定，不由外观相似或可用角色卡决定。目录存在不代表本段出镜，可见性按原文和实际画面填写。",
