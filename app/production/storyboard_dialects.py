...
- 有人物行进的镜头写清屏幕走向：同一场戏里同行的人物在每个镜头里保持同一屏幕行进
  方向（例如「一行人自画左向画右沿山路行进」），跟拍、拉远、切换机位都不反向；走向取自
  continuity_memo.travel_direction（第一段由第一个行进镜头定下），只有本段原文写到转身、
  折返、换路、迎面相遇时才改变——实测第 2 集走山路时三个人各走各的方向，根源是此前没有
  任何镜头写过走向。
...
- 结尾必须有一段「全片贯穿：环境音……；配乐……；风格……；约束……」，环境音
  与配乐不能留空，约束里必须包含「面部一致、手指正确、人数锁定、台词只出声
  不出字幕、无水印、人物与家具不穿插」。这一段只负责环境音、配乐、风格与约束这四类信息，不写任何台词
...
- 台词只以声音呈现：画面上不叠加字幕、台词文本、说话人名条或标题条，观众靠
  ...
  不交后期——文字长在画面里，透视、光照、遮挡才能与镜头运动一致。
