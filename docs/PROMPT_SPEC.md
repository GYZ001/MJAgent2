...
> 动机（真实问题）：分集→剧本→分镜每一步都在压缩，最容易被静默丢掉的就是**关键台词和关键剧情**。
...
- `key_lines`：本集绝不能丢的关键台词 3~8 条（金句/决定性对白/情绪爆点）。校验（`validate_screenplay`）确认每条都**真的写进了 `full_script_text`**（`_longest_run_ratio≥0.5`，模糊匹配容忍口语化微调），否则清单形同虚设。
...
下游消费：分镜台 prompt 注入"本集必保留关键台词/关键剧情点 + 单集戏剧契约"区块，要求逐条落实到镜头；校验器 V11（`validate_storyboard_preserves_key_content`）再逐条确认它们仍在分镜里（见 §C 校验表）。务实优先：全程模糊匹配、只拦明显丢失，避免空耗修复轮次。
