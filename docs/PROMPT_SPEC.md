...
{逐条错误，含字段路径与期望值，例如：
- shots[3].duration_s=18，但视频生成时长固定为 5s
...
每个视频镜头固定 5 秒，复杂动作、长台词和不同节拍继续拆为相邻镜头，直到完整覆盖本章剧情。
...
> 紧凑的本质是**每 5 秒都有一次局势变化**，连贯的本质是**拍与拍构成因果链**。
> 张欣指出“不能拿小说直接分镜”，本阶段即承担小说 → 可拍剧本的改编职责：先定谁在场、谁说什么、什么动作可见、局势怎样变，再让分镜做视觉翻译。

**历史说明**：本节描述的独立节拍链未接入 live 链路，不构成活动合同。当前 live 链路直接从完整剧本生成分镜大纲并逐镜填充；镜头数不由兼容时长字段推导，所有镜头固定 5 秒，按剧情、动作和口播需要继续拆分。
...
- 首尾帧纪律（v12 新增）：`first_frame_desc` 与 `last_frame_desc` 必须【同机位、同场景、同构图】，只让人物动作从开始推进到结束；二者不能完全相同（`_too_similar` ≥0.8 会退回），但绝不能变成两个不同的镜头/景别/场景——否则 5s 视频在两帧间出现反常识的跳变/形变
...
4. duration_s 固定为 5，这是最终 Seedance 视频生成参数 `--dur 5`；动作或口播在 5 秒内无法完成时，必须拆成连续相邻镜头。
...
9. action_desc 要写清一个连贯动作的起势、过程、收势和可见反应；5 秒内只完成一个可拍动作，禁止在单段里塞入多镜头快切。
...
11. 每个 5s 视频段必须推进一个明确的新动作、信息或局势变化。禁止单纯场景氛围、人物姿态、重复上一镜内容。
...
16. 单句台词可按人物语气灵活长短，但单镜台词+旁白必须能在固定 5 秒内清晰表达；关键长台词请拆成连续相邻镜头分段说。emotion 只能取：平静|愤怒|悲伤|惊恐|喜悦|讥讽|坚定。台词从原著提炼为口语化短句，但优先保留关键细节和人物说话风格：{各角色 speech_style}
...
1. 按完整覆盖剧本规划所需 shot；若单镜台词、动作或关键剧情在 5 秒内放不下，必须新增相邻镜头拆分承接。镜头数不设产品上限，每条 duration_s 固定为 5。
...
1. 先按原文顺序列出覆盖完整剧本所需的剧情段；若某段关键台词、内心OS或剧情点在 5 秒内放不下，再拆出相邻承接镜头分担，不得把同一事件拆成互相重复的摘要，也不得跳到原文后文再跳回来。
...
- 每个镜头输出前完成自检：shot_no 连续、duration_s 全部为 5、characters 非空且姓名准确、action_desc 出现准确角色名、source_excerpt 已从原文逐字摘录、scene_setting 足够短、文案满足信息密度下限且不检查上限、
...
| V2 | duration 为固定 5s 的 Seedance 合同 | `shots[i].duration_s={x}，视频生成时长固定为 5s` |
