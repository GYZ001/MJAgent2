...
| Seedance 任务完成（按分镜生成 5~10s 视频） | 未实测 | 轮询间隔 10s，总预算 **15min** | 超预算判失败，可手动重提 |
...
POST {base}/contents/generations/tasks
{
  "model": "<MODEL_VIDEO>",
  "content": [
    {"type": "text", "text": "<编译后的prompt> --ratio 9:16 --dur <5到10的分镜选择值>"},
...
- [x] **时长合法取值**：当前产品合同由分镜模型按单一动作与口播判断 **5~10s 整数**，产品侧编译器保留模型选择并输出 `--dur {duration_s}`；超过 10 秒仍演不完/说不完或包含不同节拍时必须拆成连续相邻镜头
