...
| 最高原则 | 内容、质量和结构门禁只能触发有次数上限的重试；次数耗尽后必须发布当前最佳可用产物，不得仅凭门禁结果终止生产 |

### 1.1 2026-07-31 生产决策（覆盖旧口径）

本节是本 PRD 的最新强制口径；正文中凡与本节冲突的“结构门禁阻断”“门禁失败”“必须修复后才能发布”等描述，均以本节为准：

- 内容、质量、叙事、结构、一致性等门禁只负责给出评分、告警和有限次数重试建议，不得把已有产物判为终态失败、删除或阻止进入下游。
- 有限重试次数耗尽后，文本发布最后一个可解析版本，图片发布当前最佳可读主图或部分视图，视频使用当前最佳关键帧；没有可用参考图时允许按文本提示继续生成。
- 各模块必须使用同一状态语义：`gate_retry_exhausted` 表示“重试已用尽并采用保底产物”，不是失败；其结果允许确认、生产和交付。
- 只有确实不存在任何可解析产物、媒体文件缺失/为空/不可读取、供应商不可用、鉴权/预算/并发版本冲突、显式用户取消或拒绝等非门禁问题，才可以暂停或失败。
- 所有保底放行都必须保留告警与评估证据，方便人工了解质量风险，但告警不得反向改变产物状态。
