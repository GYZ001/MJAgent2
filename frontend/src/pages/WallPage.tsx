...
export default function WallPage() {
  ...
  return (
    <div className="wall-page">
      ...
      {context && !context.upstream.eligible_for_production && <section className="review-blocked-banner" role="status"><b>当前不可生成</b><span>{context.upstream.blockers.join('；')}。查看、停止旧任务和废弃/隔离仍可用，生成、恢复、采用和修复已保护。</span><button className="btn small" onClick={() => go(ep.status === 'scripting' || ep.status === 'planned' ? 'script' : 'board', projectId, ep.id)}>去{ep.status === 'scripting' || ep.status === 'planned' ? '映射台' : '分镜台'}处理</button><details><summary>技术详情</summary><code>{context.upstream.qualification_version}</code></details></section>}
      ...
      {shots.length === 0 ? (
        <section className="review-business-empty"><div className="big">镜</div><h2>本集尚无可生成镜头</h2><p>当前状态：{episodeState.label}。{episodeState.next}。</p><div><button className="btn" onClick={() => go('script', projectId, ep.id)}>去映射台</button><button className="btn primary" onClick={() => go('board', projectId, ep.id)}>去分镜台</button><button className="btn ghost" onClick={() => { void refreshAll() }}>刷新</button></div></section>
      ) : (
        <>
