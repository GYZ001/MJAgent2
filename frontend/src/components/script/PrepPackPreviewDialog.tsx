 ...
 *
 * 这是映射台唯一一处"点下去会真正开始生成"之前的确认点，所以是说明
 ...
 *
 * 待补图数量条（第三条 <li>）同样必须排在启动按钮之前：后端 _screenplay_cast_impact
 ...
 * 无法确知（取决于模型读完原文报出什么），这里绝不能拿 0 或任何默认值顶替
 * ——把"不确定"如实转述给用户，不伪造精确数字（CLAUDE.md「不得兜底填充」）。
 */
        ...
        <ul>
          ...
          {stage && (
            <li className="prep-cost-preview">
              ...
              共 {stage.known_image_count ?? 0} 张
              （已有参考图的角色/场景自动复用，不重复生成）。
              <br />
          ...
          {preview.data.blueprint_budget?.requires_fresh_retry_grant && (
            <li className="danger">
              ...
              为避免同一环节被重复触发，系统已暂停自动重试；确认继续将授权对同一环节重新发起一次调用。
            </li>
          )}
        </ul>
