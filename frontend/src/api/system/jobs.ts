...
/** 供应商终态拒绝、且确凿未产生任何调用——给 JobDrawer 判断要不要展示「释放
 *  占用」按钮，以及确认文案里要写的理由。不满足也带结构化 reason，
 *  不把用户晾在原地。 */
...
/** 本地二段式确认：不带 confirm 只预览（服务端会返回将要释放的清单，
 *  抛 422 供 JobDrawer 展示确认文案），带 confirm=true 才真正释放占用。 */
export function releaseZeroCostJobs(
  jobIds: string[],
  confirm: boolean,
): Promise<{ ok: boolean; released: Array<{ job_id: string; reason: string }> }> {
  return request(
    "POST",
