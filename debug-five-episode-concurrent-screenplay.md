...
## Round 7-14：把猜测变成事实，以及两次被证伪的假设

### 事实：那 22 个字符是什么

给中断流加了有界证据留存（`_interrupted_stream_evidence`）之后一次就拿到了：

```
抱歉，该问题不符合安全合规要求，暂时无法回答
```

正好 22 个字符，以流内容形式发出后不发 `[DONE]` 直接断流。

### 被证伪的假设一：内容安全拒绝

按「安全拒绝」去改提示词（把「审查/违规/定罪」这类裁决口吻换成创作校对说法），
拒绝率没有下降（19/93 → 16/50）。

### 真正的原因：并发高峰下的限流丢弃

按时间轴排列同一轮的审查调用：前 70 秒 14 次全部成功，70–89 秒集中爆发拒绝，
之后又恢复正常。**拒绝在时间上高度聚集，与提示词内容、长度、max_tokens 都不相关，
而且只发生在 `screenplay_scene_shard_semantic_review` 这一个阶段**
（同样携带故事内容的场次写作 0/22、人物识别 0/16、全局包络 0/6）。

之前看到的「同一 prompt 4/4 失败」是 4 次尝试都落在同一个丢弃窗口里，
不是内容决定的。相应地把 `deterministic_undelivered_error` 的文案从
「重试不会成功」改成「通常是并发高峰下的限流丢弃，请降低并发」。

### 被证伪的假设二：靠退避等过丢弃窗口

试过两个位置，都失败，且都造成了比原问题更糟的后果：

1. **租约内 4s+16s**：退避写在供应商槽位租约里，长时间空占仅有的几个槽位，
   把其它阶段饿死，直接导致三集的蓝图阶段被整个取消。已收紧到 1.5s+4.0s。
2. **块级 8s/25s/60s**（写在租约之外，本以为安全）：分片任务在 sleep 期间
   仍然占着有界的 workflow/provider 池，结果**连从未进入审查阶段的 EP1/EP4 蓝图**
   也耗尽各自的时钟预算被取消，五集全灭。已整个撤回。

结论写进代码注释：**在这个共享有界池 + 每阶段时钟预算的管线里，任何地方增加延迟
都会被所有阶段一起买单**，唯一负担得起的是租约内那个很短的退避。

把 `text_generation_concurrency` 从 10 降到 6 也没有降低拒绝率（32% → 26%），
说明丢弃不完全由本客户端并发决定（很可能是供应商侧共享负载），随后已恢复为 10。

### 仍然存在的报告缺陷

批次 fail-fast 触发时，运行以 `CANCELLED` 且 `failure_code=None` 收尾，
真正的首因被 `CancelledError` 吞掉——和根因 8 是同一类问题，只是发生在 run 这一层。
