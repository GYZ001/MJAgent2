...
后端默认使用稳定模式，不监听源码文件即时重载；`scripts/backend_cycle.py` 每 1800 秒统一重启一次，
避免连续保存多个文件时反复中断服务。可用 `MJ_BACKEND_RESTART_INTERVAL_SECONDS` 调整周期；
只有显式设置 `MJ_BACKEND_RELOAD=1` 才会启用实时热加载。
