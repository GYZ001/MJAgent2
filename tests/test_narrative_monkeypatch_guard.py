"""Guard against the app.narrative package-split monkeypatch trap.

``app/narrative.py`` used to be one file; every call site inside it shared a
single module namespace, so ``monkeypatch.setattr(narrative, "name", value)``
(or the string form ``monkeypatch.setattr("app.narrative.name", value)``, or
a direct ``narrative.name = value`` assignment) reached every caller. It was
split into the ``app.narrative`` package (see ``app/narrative/__init__.py``);
each submodule now holds its own copy of any name it imported (``from
.primitives import _norm`` and friends). Patching only the package-level
...
The fix is ``tests/conftest.py``'s ``patch_narrative_everywhere(monkeypatch,
name, value)`` -- it walks every ``app.narrative`` submodule and patches
...
``patch_narrative_everywhere``'s own implementation (which *is* the one
...
shared singleton module object reached via the narrative package (e.g.
``narrative.config``, a plain ``from app import config`` re-export) is never
affected by the package split -- patching an attribute *on that shared
object itself* (``monkeypatch.setattr(narrative.config, "SOME_FLAG",
value)``) mutates the one object every submodule holds a reference to. Only
...
AST check below distinguishes the two: ``narrative.foo`` (an ``Attribute``
node) is never flagged, only bare ``narrative`` (a ``Name`` node) or an
``"app.narrative.<single identifier>"`` string.
...
``monkeypatch.setattr(narrative, ...)`` call sites. A loop such as ``for
module in (a, b, narrative): monkeypatch.setattr(module, ...)`` binds
``narrative`` into a loop variable first and is invisible to this scan. Any
...
HELPER_NAME = "patch_narrative_everywhere"

BARE_NAME_ALIASES = {"narrative"}


def _helper_exempt_span(tree: ast.Module) -> tuple[int, int]:
    """Line range of patch_narrative_everywhere's own body in conftest.py.

    This is the sole legitimate place a bare ``setattr(narrative, name,
    value)`` / ``narrative.<x> = ...`` may exist -- it *is* the
...
def _is_bare_narrative_name(node: ast.expr) -> bool:
    return isinstance(node, ast.Name) and node.id in BARE_NAME_ALIASES


def _is_bare_narrative_attr_string(node: ast.expr) -> bool:
    """True for ``"app.narrative.<single identifier>"``.

    Not ``"app.narrative"`` itself (patching the module object as a whole is
    a different, rarer operation) and not a deeper dotted path such as
    ``"app.narrative.config.SOME_FLAG"`` -- that patches a real shared
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(node, ast.Call):
            ...
            if (is_setattr_call or is_patch_object_call) and node.args:
                target = node.args[0]
                if _is_bare_narrative_name(target):
                    violations.append(
                        f"{path}:{node.lineno}: bare narrative-package attribute "
                        "patch (monkeypatch.setattr(narrative, ...) / "
                        "patch.object(narrative, ...)) only reaches "
                        "app.narrative's own re-export -- app/narrative is a "
                        ...
                        "tests.conftest.patch_narrative_everywhere(monkeypatch, "
                        "name, value) instead."
                    )
            if (is_setattr_call or is_patch_call) and node.args:
                target = node.args[0]
                if _is_bare_narrative_attr_string(target):
                    violations.append(
                        f"{path}:{node.lineno}: string-form patch on "
                        f"{ast.literal_eval(target)!r} only reaches "
                        "app.narrative's own re-export, not the submodule that "
...
def test_no_bare_app_narrative_package_monkeypatch() -> None:
