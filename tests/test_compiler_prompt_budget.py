...
def test_long_reference_prompt_compacts_without_losing_story_anchors() -> None:
    ...
    action = (
            ...
            "周围人群骚动，functional:heckler-a摇头，functional:heckler-b窃语。甲一按在石碑上的手收紧。"
    )
    shot = Shot(
        shot_no=2,
        duration_s=8,
        ...
        characters=["甲一", "functional:heckler-a", "functional:heckler-b", "functional:examiner"],
        action_desc=action,
        first_frame_desc="甲一按住石碑，functional:examiner低头等待结果。",
        last_frame_desc="functional:examiner公布成绩，甲一按碑的手收紧。",
        state_in="甲一按住石碑，functional:examiner低头等待结果。",
        primary_action=action,
        state_out="functional:examiner公布成绩，甲一按碑的手收紧。",
        continuity_mode="same_scene_cut",
        source_excerpt=(
            "测验员看了一眼碑上所显示出来的信息，语气漠然地将之公布了出来。"
            "中年男子话刚刚脱口，便在人头汹涌的广场上带起一阵嘲讽的骚动。" * 3
        ),
        dialogues=[Dialogue(speaker="functional:examiner", line="甲一，测验力，三段！级别：低级！", emotion="平静")],
    ...
    assert "甲一，测验力，三段！级别：低级！" in prompt
...
def test_silent_shot_compacts_without_forcing_dialogue_pacing(monkeypatch) -> None:
    ...
    action = (
        "甲一抬手按上冰冷石碑，因用力而发白，碑面光纹向外扩散，甲二儿侧身注视，"
        "甲四立于边缘压抑呼吸，三人神情绷紧，动作连贯不跳切。"
    ) * 3
    shot = Shot(
        shot_no=3,
        duration_s=10,
        ...
        camera_move="推近",
        scene_setting="日，甲家测验广场",
        characters=["甲一", "甲二儿", "甲四"],
        action_desc=action,
        first_frame_desc="甲一按碑，甲二儿侧视，甲四立于边缘。",
        last_frame_desc="光纹铺满碑面，三人神情更紧。",
        state_in="甲一按碑，甲二儿侧视，甲四立于边缘。",
        primary_action=action,
...
def test_large_continuity_snapshot_projects_current_pose_and_changed_props() -> None:
    ...
    shot = Shot(
        ...
        shot_size="中景",
        camera_move="推近",
        scene_setting="白天，甲家测验广场",
        characters=["甲一"],
        characters_visible=["甲一"],
        action_desc="甲一右掌按住测验石碑，碑面光纹由暗转亮。",
        first_frame_desc="甲一右掌贴住暗淡的测验石碑。",
        last_frame_desc="甲一肩背绷紧，测验石碑表面光纹亮起。",
        state_in="甲一右掌贴住暗淡的测验石碑。",
        primary_action="甲一按住测验石碑使碑面光纹亮起",
        state_out="甲一肩背绷紧，测验石碑表面光纹亮起。",
        continuity_mode="same_scene_cut",
        continuity_state_in=ContinuityState(
            scene=scene,
            characters={"甲一": character_in},
            props={**stable_props, "测验石碑": current_prop_in},
        ),
        continuity_state_out=ContinuityState(
            scene=scene,
            characters={"甲一": character_out},
