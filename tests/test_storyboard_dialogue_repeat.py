...
def test_exact_repeat_by_same_speaker_is_rejected_and_names_the_prior_segment():
    ...
    errors = repeated_delivery_errors(delivered, current, current_segment_no=6, reserved=[], required_texts=[])
    assert len(errors) == 1
...
def test_different_speaker_saying_the_same_line_is_not_flagged():
    ...
    assert repeated_delivery_errors(delivered, current, current_segment_no=6, reserved=[], required_texts=[]) == []
...
def test_multiple_delivered_segments_report_the_correct_earlier_segment_number():
    ...
    errors = repeated_delivery_errors(delivered, current, current_segment_no=7, reserved=[], required_texts=[])
...
def test_empty_delivered_history_never_flags_the_first_segment():
    ...
    assert repeated_delivery_errors([], current, current_segment_no=1, reserved=[], required_texts=[]) == []
...
def test_multiple_repeats_in_one_segment_each_produce_their_own_error():
    ...
    errors = repeated_delivery_errors(delivered, current, current_segment_no=2, reserved=[], required_texts=[])
    assert len(errors) == 2
...
def test_preempting_a_later_segment_required_line_is_blocked_even_when_paraphrased() -> None:
    ...
    errors = repeated_delivery_errors(
        [], [("bible:李麦麦", "跟我去公司，别出声。")], current_segment_no=4, reserved=reserved, required_texts=[],
    )
...
def test_short_or_unrelated_lines_do_not_count_as_preemption() -> None:
    ...
    errors = repeated_delivery_errors(
        ...
        current_segment_no=4, reserved=reserved, required_texts=[],
    )
    assert errors == []
