...
def test_exact_repeat_by_same_speaker_is_rejected_and_names_the_prior_segment():
    ...
    errors = repeated_delivery_errors(delivered, current, current_segment_no=6)
    assert len(errors) == 1
...
def test_different_speaker_saying_the_same_line_is_not_flagged():
    ...
    assert repeated_delivery_errors(delivered, current, current_segment_no=6) == []
...
def test_multiple_delivered_segments_report_the_correct_earlier_segment_number():
    ...
    errors = repeated_delivery_errors(delivered, current, current_segment_no=7)
...
def test_empty_delivered_history_never_flags_the_first_segment():
    ...
    assert repeated_delivery_errors([], current, current_segment_no=1) == []
...
def test_multiple_repeats_in_one_segment_each_produce_their_own_error():
    ...
    errors = repeated_delivery_errors(delivered, current, current_segment_no=2)
    assert len(errors) == 2
