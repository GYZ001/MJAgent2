...
#: api.py 是 L5（REST 路由，需要 app.auth.principal/app.orgs.store），其余六个
#: 模块（schema/plans/allocation/usage_query/storage/fair_ordering）是 L2，与
#: app.quota 同层。fair_ordering 是 2026-09-13 新增（EP-04 §6 接入调度器）。
EXPECTED_LAYERS = {
    "app.quota_policy": 2,
    ...
    "app.quota_policy.fair_ordering": 2,
    "app.quota_policy.api": 5,
}

#: 允许反向依赖 app.quota 的唯一例外——api.py 是 L5 路由层，本身不禁止依赖
#: app.quota（派单红线明确针对"新包"整体，api.py 目前也确实没有这么写，但
#: 架构上它不像 schema/plans/allocation/usage_query/storage/fair_ordering 那样
#: 承担"app.quota 的取值来源"角色，未来即便真的需要也不构成本次红线要防的那种
#: 循环）。
_FORBIDDEN_REVERSE_DEP_MODULES = (
    "schema.py", "plans.py", "allocation.py", "usage_query.py", "storage.py", "fair_ordering.py",
)
