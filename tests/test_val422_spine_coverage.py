...
def _screenplay_with_spine() -> EpisodeScreenplay:
    return EpisodeScreenplay(
        ...
        plot_spine=PlotSpine(
            ...
            must_keep_ending="二儿追上",
            drop_list=["无关支线"],
        ),
        information_ledger=[
            InformationItem(
                info_id="I3.1", content="甲三测出测验力七段",
                delivery_owner="visual_action",
            ),
            InformationItem(
                info_id="I3.3", content="甲三望向甲一后选择不过去",
                delivery_owner="visual_action",
            ),
        ],
    )
...
def test_s04_split_across_adjacent_shots_passes() -> None:
    ...
    s5.characters = ["甲三"]
    s5.characters_visible = ["甲三"]
    s5.information_ids = ["I3.1"]
    s5.action_desc = "甲三上前测出测验力七段，碑面亮起，人群追捧赞叹"
    s5.dialogues = []
    s6 = _compact_shot(6)
    s6.spine_beat_ids = ["S04"]
    s6.characters = ["甲三"]
    s6.characters_visible = ["甲三"]
    s6.information_ids = ["I3.3"]
    s6.action_desc = "甲三望向后排甲一一眼后转身走开，不再过去"
    s6.dialogues = []
    ...
    s9.action_desc = "二儿上前握住甲一的手，认真安慰他"
...
def test_spine_id_cannot_replace_visible_action_subject() -> None:
    ...
    s9.spine_beat_ids = ["S07"]
    s9.key_line_ids = ["KL02"]
    s9.action_desc = "二儿走到甲一面前，认真安慰甲一。"
    s9.dialogues = [
        Dialogue(
            speaker="二儿",
            line="二儿相信，你会重新站起来，取回属于你的荣耀与尊严",
    ...
    assert any("主线节拍缺少可见动作主体" in error and "S04/甲三" in error for error in errs), errs
...
def test_s07_split_across_shots_passes() -> None:
    ...
    s5.characters = ["甲三"]
    s5.characters_visible = ["甲三"]
    s5.action_desc = "甲三测出测验力七段，人群赞叹；望向甲一后选择不过去"
    s5.information_ids = ["I3.1", "I3.3"]
    s9 = _compact_shot(9)
    s9.spine_beat_ids = ["S07"]
    s9.key_line_ids = ["KL02"]
    ...
    s9.action_desc = "二儿安慰甲一"
    s10 = _compact_shot(10)
    s10.spine_beat_ids = ["S07"]
    s10.action_desc = "甲一转身离场，二儿追上与他并肩而行"
    s10.dialogues = []
...
def test_spine_mixed_visible_and_spoken_delivery_uses_the_right_evidence() -> None:
    """真实回归：丙老现身是可见动作，背景暗示由对白交付，不能整句做字面动作匹配。"""
    screenplay = EpisodeScreenplay(
        episode_no=1,
        plot_spine=PlotSpine(
            episode_premise="丙老提醒甲一留意二儿的来历",
            ...
            must_keep_ending="甲一开始追问",
            drop_list=[],
        ),
        key_lines=["丙老：那小丫头，来历似乎有点不一般啊。"],
    )
    shot = _compact_shot(1)
    shot.spine_beat_ids = ["S02"]
    shot.key_line_ids = ["KL01"]
    shot.characters = ["甲一", "丙老"]
    shot.characters_visible = ["甲一", "丙老"]
    shot.primary_action = "甲一放下卷轴，丙老现身开口提及二儿来历"
    shot.action_desc = "甲一把卷轴放在桌上，丙老半透明身影从戒指上方浮现于桌旁"
    shot.first_frame_desc = "夜晚卧室内，甲一站在桌前，戒指发出微光"
    shot.last_frame_desc = "同一机位，丙老半透明身影已经悬浮在木桌右侧"
    ...
    assert not any("S02/丙老" in error for error in errors), errors
...
def test_spine_spoken_delivery_still_requires_the_subject_to_speak() -> None:
    screenplay = EpisodeScreenplay(
        ...
        plot_spine=PlotSpine(
            episode_premise="丙老提醒甲一留意二儿的来历",
            spine_beats=[PlotSpineBeat(
                beat_id="S02", who="丙老", does="现身并暗示二儿背景不凡",
                turn="甲一开始追问", must_keep=True,
            )],
            must_keep_ending="甲一开始追问", drop_list=[],
        ),
    )
    shot = _compact_shot(1)
    shot.spine_beat_ids = ["S02"]
    shot.characters = ["丙老"]
    shot.characters_visible = ["丙老"]
    shot.primary_action = "丙老现身"
    shot.action_desc = "丙老半透明身影从戒指上方浮现于桌旁，随后保持沉默"
    shot.dialogues = []
    ...
    assert any("S02/丙老" in error for error in errors), errors
...
def test_assign_outline_delivery_ids_from_covers() -> None:
    ...
    outline = StoryboardOutline(
        ...
        shots=[
            StoryboardOutlineShot(
                shot_no=1,
                beat="甲三测出七段并望向甲一后不过去",
                covers="甲三测出七段测验力收获追捧，望向甲一后选择不过去",
                duration_s=5,
            ),
            StoryboardOutlineShot(
                shot_no=2,
                beat="二儿安慰",
                covers="二儿相信，你会重新站起来，取回属于你的荣耀与尊严",
                duration_s=10,
            ),
        ],
    )
